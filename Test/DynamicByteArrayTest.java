...
public class DynamicByteArrayTest {
    ...
    public void test_emptyGet() throws Exception {
        ...
        Assert.assertEquals(10, b1.length);  // should be requested length
        Assert.assertEquals(1010, arr.getCurrentSize()); // get should cause growing
    }

    @Test
    public void test_shrink() throws Exception {
        DynamicByteArray arr = new DynamicByteArray();
        Assert.assertEquals(0, arr.getCurrentSize()); // ampty dba has size 0
        arr.insert(10, new byte[]{1, 2, 3, 4, 5});
        Assert.assertEquals(15, arr.getCurrentSize()); // shoud be 15 now
        arr.delete(0, 0);
        Assert.assertEquals(15, arr.getCurrentSize()); // shoud be 15 now
        arr.delete(0, 15);
        Assert.assertEquals(0, arr.getCurrentSize()); // shoud be 0 now
        arr.insert(0, new byte[]{8, 9, 10, 42, 54});
        Assert.assertEquals(5, arr.getCurrentSize()); // shoud be 5 now
        arr.delete(0, 100);
        Assert.assertEquals(0, arr.getCurrentSize()); // attempt to del > size, shoud be 0 now
    }
    ...
    public void dynBAPut() throws Exception {
        ...
        db.put(102, b2);
        bg = db.get(100, 7);
        Assert.assertArrayEquals(new byte[]{1, 2, 5, 6, 7, 8, 9}, bg);
        db.undo();
        bg = db.get(100, 4);
        Assert.assertArrayEquals(b1, bg);
    }
    ...
    public void splitTest() throws Exception {
        ...
        Assert.assertEquals(10, sp1.length);
        Assert.assertEquals(11, sp2.length);
        Assert.assertEquals(11, sp3.length);
